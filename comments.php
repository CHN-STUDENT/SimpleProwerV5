    ...
    <?php if($this->allow('comment')): ?>
        <div id="<?php $this->respondId(); ?>" class="respond">
            ...
            <form method="post" action="<?php $this->commentUrl() ?>" id="comment_form" class="comment-list">
                ...
                <?php endif; ?>
                <p><textarea rows="3" cols="50" name="text" id="textarea" placeholder="添加一条评论" required ><?php $this->remember('text'); ?></textarea><div><button type="submit" class="submit">发表评论</button></p></div>
            </form>
			<hr />
        </div>
    <?php else: ?>
